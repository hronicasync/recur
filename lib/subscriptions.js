...
export const findSubscriptionByNameAndDate = async ({ userId, name, nextDue }) => {
  const { rows } = await query(
    `select id, user_id, name, amount, currency, period, next_due, reminders, emoji
       from subscriptions
...
export const createSubscription = async ({
  ...
  const { rows } = await query(
    `insert into subscriptions (user_id, name, amount, currency, period, next_due, reminders, emoji, notes)
     values ($1, $2, $3, $4, $5, $6, $7, $8, $9)
     returning id, user_id, name, amount, currency, period, next_due, reminders, emoji, notes`,
    [userId, name, amount, currency, period, nextDue, JSON.stringify(reminders), emoji || '▫️', notes || null],
  );
...
export const updateSubscription = async (id, updates = {}) => {
  ...
  if (fields.length === 0) {
    const { rows } = await query(
      'select id, user_id, name, amount, currency, period, next_due, reminders, emoji, notes from subscriptions where id = $1',
      [id],
    );
    return rows[0] || null;
  }
...
export const shiftNextDue = async ({ id, userId, nextDue }) => {
  const { rows } = await query(
    `update subscriptions
      ...
      returning id, user_id, name, amount, currency, period, next_due, reminders, emoji`,
    [nextDue, id, userId],
  );
  return rows[0] || null;
};
